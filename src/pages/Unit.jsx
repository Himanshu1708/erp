...
const UnitManagement = () => {
  ...
  const [units, setUnits] = useState([]); // Changed from taxes to units
  const [loading, setLoading] = useState(true);
  const [editingUnit, setEditingUnit] = useState(null); // Changed from editingTax
  const [isViewMode, setIsViewMode] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");

  // State for form data - Matching your Units entity and Controller params
  ...
  // Open modal for viewing unit
  const openViewModal = (unitData) => {
    setEditingUnit(unitData);
    setIsViewMode(true);
  ...
  // Handle form submission (create/update unit)
  const handleSubmit = async (e) => {
    e.preventDefault();
    
    // Basic validation
    if (!formData.unit) {
      toast.error("Unit Name is required", { position: 'top-right' });
      return;
    }
     // 🔴 DUPLICATE UNIT CHECK (UI LEVEL)
  const currentUnit = formData.unit.trim().toLowerCase();

  const isDuplicate = units.some(unit => {
    // while editing → ignore same unit
    if (editingUnit && unit.id === editingUnit.id) {
      return false;
    }
    return unit.unit?.trim().toLowerCase() === currentUnit;
  });

  if (isDuplicate) {
    toast.error("Unit already exists", { position: "top-right" });
    ...
    try {
      if (editingUnit) {
        ...
        // Controller uses @RequestParam names: update_unit, update_description
        const params = {
          update_unit: formData.unit,
          update_description: formData.description
        };
        
        // Axios sends params as query string for @RequestParam
        const response = await api.put(`/unit/updateUnit/${editingUnit.id}`, null, { params });
        ...
        if (response.status === 200) {
          ...
          toast.error(response.data || 'Failed to update unit', { position: 'top-right' });
        }
      } else {
        // Create new unit
        // Controller uses @RequestParam names: unit, description
        const params = {
          unit: formData.unit,
          description: formData.description
        };
        
        const response = await api.post('/unit/addUnits', null, { params });
        ...
        if (response.status === 200) {
          ...
          toast.error(response.data || 'Failed to add unit', { position: 'top-right' });
        }
      }
    } catch (error) {
      console.error('Error saving unit:', error);
      // Handle specific backend error messages if any
      toast.error(error.response?.data || 'Failed to save unit', { position: 'top-right' });
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    saveAs(fileData, `Unit_Management_${Date.now()}.xlsx`);
    toast.success("Excel file exported successfully");
  };

  // Handle unit deletion - Direct delete without confirmation
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        toast.error(response.data || 'Failed to delete unit', {
          position: 'top-right',
        });
      }
    } catch (error) {
  ...
  const filteredUnits = units.filter((unit) => {
    ...
    return (
      (unit.unit && unit.unit.toLowerCase().includes(search)) ||
    ...
    <div style={styles.container}>
      ...
      <main style={styles.mainContent}>
        <div style={styles.header}>
          <h1 style={styles.headerTitle}>Unit Management</h1>
        ...
        <div style={styles.tableContainer}>
          <div style={styles.tableHeader}>
            <div style={styles.tableTitle}>All Units</div>
            ...
            <table style={styles.table}>
              ...
              <tbody>
                {filteredUnits.length > 0 ? filteredUnits.map((unit, index) => {
                  ...
                  const isHovered = hoveredRow === unit.id;
                  ...
                  <tr 
                    ...
                    onMouseEnter={() => setHoveredRow(unit.id)}
                    onMouseLeave={() => setHoveredRow(null)}
                  >
                    <td style={styles.td}>{index + 1}</td>
                    <td style={styles.td}>{unit.unit || ""}</td>
          ...
          <div style={styles.modalContent}>
            ...
            <div style={styles.modalBody}>
              <form onSubmit={handleSubmit}>
                {/* Unit ID - Read-only for edit/view */}
                {editingUnit && (
                  <div style={styles.formGroup}>
                    <label style={styles.formLabel} htmlFor="id">Unit ID</label>
                    <input
                      type="text"
                      id="id"
                      name="id"
                ...
                {/* Unit Name */}
                <div style={styles.formGroup}>
                  <label style={styles.formLabel} htmlFor="unit">Unit Name</label>
                  <input
                    type="text"
                    id="unit"
                    name="unit"
